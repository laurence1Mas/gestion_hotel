...
import { Hotel, LayoutDashboard, CalendarDays, Search, User, Settings, LogOut, Heart, History,X } from "lucide-react"
...
const clientMenuItems = [
  ...
  { href: "/dashboard/client/reservations", icon: CalendarDays, label: "Mes Réservations" },
  { href: "/hotels", icon: Search, label: "Chercher Hôtel" },
...
const hotelMenuItems = [
  ...
  { href: "/dashboard/hotel/reservations", icon: CalendarDays, label: "Réservations" },
...
export function DashboardSidebar({ type, isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={cn(
        "fixed top-0 left-0 z-50 h-full w-72 bg-card border-r border-border flex flex-col transition-transform duration-300 lg:translate-x-0 lg:static lg:z-0",
