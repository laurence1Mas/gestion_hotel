...
export const HotelDetailModal = () => {
  ...
  const total = Math.round(room.pricePerNight * nights);

  const handleBook = () => {
    if (!checkIn || !checkOut || nights <= 0) return;
    addReservation({
      hotelId: hotel.id, hotelName: hotel.name, roomId: room.id, roomName: room.name,
      userId: user.id, userName: user.name, checkIn, checkOut, guests, total, status: "confirmed",
    });
    setBooked(true);
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-scale-in" role="dialog" aria-modal="true">
      <div className="absolute inset-0 bg-primary/60 backdrop-blur-sm" onClick={closeHotel} />
      <div className="relative bg-card rounded-3xl shadow-[var(--shadow-elegant)] max-w-6xl w-full max-h-[92vh] overflow-y-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-0">
          ...
          <div className="p-6 lg:p-8">
            ...
            <div className="mt-6 relative aspect-[2/1] rounded-2xl overflow-hidden border border-border bg-secondary">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <p className="mt-3 text-xs font-medium text-foreground bg-background/90 backdrop-blur px-3 py-1 rounded-full">
                  {hotel.coordinates.lat.toFixed(2)}, {hotel.coordinates.lng.toFixed(2)}
                </p>
              </div>
          ...
          <div className="p-6 lg:p-8 lg:pl-2">
            ...
            <h2 className="font-display text-3xl lg:text-4xl text-foreground mb-3">{hotel.name}</h2>
            <div className="flex items-center gap-3 mb-4">
              <div className="flex items-center gap-1">
                <Star className="w-4 h-4 fill-accent text-accent" />
                <span className="font-semibold">{hotel.rating}</span>
                <span className="text-sm text-muted-foreground">({hotel.reviews} avis)</span>
              </div>
              <div className="flex gap-1.5 flex-wrap">
                {hotel.tags.map(t => <span key={t} className="chip">{t}</span>)}
              </div>
            </div>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground flex items-center gap-1">
                        <Users className="w-3 h-3" /> {r.capacity} pers · {r.available} dispo
                      </p>
                    </div>
                    <p className="font-display text-lg">{Math.round(r.pricePerNight)}€</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="bg-secondary/50 rounded-2xl p-5 border border-border">
              {booked ? (
                <div className="text-center py-4">
                  ...
                  <p className="font-display text-xl mb-1">Réservation confirmée</p>
                  <p className="text-sm text-muted-foreground">Retrouvez-la dans votre dashboard.</p>
                </div>
              ) : (
                <>
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div>
                      ...
                      <input type="date" value={checkIn} onChange={e => setCheckIn(e.target.value)} className="input-luxe mt-1" />
                    </div>
                    <div>
                      <label className="text-xs text-muted-foreground">Départ</label>
                      <input type="date" value={checkOut} onChange={e => setCheckOut(e.target.value)} className="input-luxe mt-1" />
                    </div>
                  </div>
                  <div className="mb-3">
                    <label className="text-xs text-muted-foreground">Voyageurs</label>
                    <input type="number" min={1} max={room.capacity} value={guests} onChange={e => setGuests(+e.target.value)} className="input-luxe mt-1" />
                  </div>
                  ...
                  <button onClick={handleBook} disabled={!nights} className="btn-gold w-full disabled:opacity-50 disabled:cursor-not-allowed">
                    Réserver maintenant
                  </button>
                </>
              )}
