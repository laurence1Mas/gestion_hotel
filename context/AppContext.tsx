...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addReservation = useCallback((r: Omit<Reservation, "id" | "createdAt">) => {
    setReservations(prev => [
      { ...r, id: `r${Date.now()}`, createdAt: new Date().toISOString().slice(0, 10) },
      ...prev,
    ]);
  }, []);
  ...
  const value = useMemo(
    () => ({ user, setUser, hotels, reservations, selectedHotel, openHotel, closeHotel, addReservation, cancelReservation }),
    [user, hotels, reservations, selectedHotel, openHotel, closeHotel, addReservation, cancelReservation]
  );
